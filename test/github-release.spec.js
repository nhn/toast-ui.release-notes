...
describe('tui-release-note', () => {
  ...
  describe('getRepositoryUrl()', () => {
    ...
    it('should throw error, if repository property is invalid', () => {
      expect(() => {
        tnote.getRepositoryUrl('');
      }).toThrow('repository in package.json is invalid.');
      expect(() => {
        tnote.getRepositoryUrl({});
      }).toThrow('repository in package.json is invalid.');
    });
  ...
  describe('filterCommits()', () => {
    ...
    it('should add commit object, when commit message matches with regexp', () => {
      ...
      expect(commitObject.sha1).toBe('qwertyu');
      expect(commitObject.type).toBe('Fix');
      expect(commitObject.title).toBe('some issue (fix: #11)');
  ...
  describe('commit object creation', () => {
    const commitLog =
      '* 91dccdf Feat: title (resolves #22, #23) (#26) (author)\n' +
      '* 3bebcfd Refactor: title (#8) (future worker)\n' +
      '* e639358 Fix: bug (fixes #21) (#25) (fixer)\n';

    it('should make commit object by capture group', () => {
      const captureGroup = ['', '91dccdf', 'Feat', 'title (resolves #22, #23) (#26)', '(author)'];
      ...
      expect(commitObject.author).toBe('(author)');
    });
