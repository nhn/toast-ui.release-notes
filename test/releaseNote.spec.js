...
describe('tui-release-note', () => {
  describe('filterCommits()', () => {
    ...
    it('should add commit object, when commit message matches with regexp', () => {
      const commitObjects = tuiNote.filterCommits([validCommitObject]);
    ...
    it('should not add commit object, when commit message does not match with regexp', () => {
      const commitObjects = tuiNote.filterCommits([invalidCommitObject]);
  ...
  describe('getGroupsByCommitType()', () => {
    it('should group by its type.', () => {
      expect(tuiNote.getGroupByCommitType('fix: some issue (fix: #11)')).toBe('Bug Fixes');
      expect(tuiNote.getGroupByCommitType('feat: some features (ref: #12)')).toBe('Features');
      expect(tuiNote.getGroupByCommitType('refactor: refactoring')).toBe('Enhancement');
      expect(tuiNote.getGroupByCommitType('perf: improve performance')).toBe('Enhancement');
      expect(tuiNote.getGroupByCommitType('docs: add tutorials')).toBe('Documentation');
      expect(tuiNote.getGroupByCommitType('chore: apply code review')).toBe('');
    });
  });
  ...
  describe('should make a release note', () => {
    ...
    it('by commits.', () => {
      const notes = tuiNote.renderCommits([commitObject1, commitObject2]);
      expect(notes).toBe(
    ...
    it('by downloads.', () => {
      const notes = tuiNote.renderDownloads(links);
      expect(notes).toBe('* [js](tui-release-note.js)\n* [css](tui-release-note.css)\n');
    });

    it('with grouping commits.', () => {
      const releaseNote = tuiNote.makeReleaseNote([commitObject1, commitObject2]);
      const expected =
        '\n## Features\n\n* 91dccdf Feat: a new feature (ref #31)\n' +
        '\n## Enhancement\n\n* 3bebcfd Perf: improve the performance\n';
